...
public class LiveWallpaper extends WallpaperService
{
			...
			SharedPreferences.OnSharedPreferenceChangeListener
	{
		...
		{
            c.save();
            BitmapDrawable ball = (BitmapDrawable) getBaseContext().getResources().getDrawable(R.drawable.logo);
            if (x<0 && y <0) {
                x = mRectFrame.width()/2;
                y = mRectFrame.height()/2;
            } else {
                x += xVelocity;
                y += yVelocity;
                if ((x > mRectFrame.width() - ball.getBitmap().getWidth()) || (x < 0)) {
                    xVelocity = xVelocity*-1;
                }
                if ((y > mRectFrame.height() - ball.getBitmap().getHeight()) || (y < 0)) {
